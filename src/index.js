...
const App = {
	...
	lookUp: async function () {
		...
		const { lookUptokenIdToStarInfo, lookUptokenIdToStarSymbol } = this.meta.methods;
		let name = await lookUptokenIdToStarInfo(lookid).call();
		let symbol = await lookUptokenIdToStarSymbol(lookid).call();
		App.setStatus(`Name: ${name}  -- Symbol: ${symbol}`);
	},
};
